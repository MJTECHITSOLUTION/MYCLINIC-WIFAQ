            ...
            <div class="row">
               <div class="col" >
                  @if(!empty(App\Setting::get_option('logo')))
                     <img src="{{ asset('uploads/'.App\Setting::get_option('logo')) }}" style="width: 50%;"><br><br>
                  @endif
                       ...
                       @if(isset($billing_items) && count($billing_items) > 0)
                           @foreach($billing_items as $billing_item)
                               <tr>
                                   ...
                                   <td>{{ isset($billing_item->invoice_amount) ? $billing_item->invoice_amount . ' ' . App\Setting::get_option('currency') : 'N/A' }}</td>
                                   <td>{{ isset($billing_item->payer) ? $billing_item->payer : 'N/A' }}</td>
                                   <td></td>
                                   @php
                                       $totalbilling += $billing_item->invoice_amount;
                                       $restbilling = $billing_item->payer - $billing_item->invoice_amount;
