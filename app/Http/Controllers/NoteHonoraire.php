...
{
    $note = Note::findOrFail($id);
    $notes = Honoraire::where('not_id', $id)->select('*')->addSelect('id as honorire_id')->get();

    $acts = Act::all();

    $patient = User::where('id', $note->user_id)->first();

    return view('notehonoraire.edit', compact('note', 'patient', 'acts','notes'));
}
public function update(Request $request, $id)
{
    // Validate the input data
    $request->validate([
        'old_act_name.*' => 'nullable|string',
        'old_code.*' => 'nullable|string',
        'old_lettrecle.*' => 'nullable|string',
        'old_date.*' => 'nullable|date',
        'old_dents.*' => 'nullable|string',
        'old_montant.*' => 'nullable|numeric',
        'act_name.*' => 'nullable|string',
        'code.*' => 'nullable|string',
        'lettrecle.*' => 'nullable|string',
        'date.*' => 'nullable|date',
        'dents.*' => 'nullable|string',
        'montant.*' => 'nullable|numeric',
    ]);

    // Update existing rows
    if ($request->has('old_id')) {
        $oldIds = $request->input('old_id');

        // Ensure $oldIds is an array
        if (!is_array($oldIds)) {
            $oldIds = [$oldIds];
        }

        foreach ($oldIds as $index => $oldId) {
            $oldId = (int) $oldId; // Explicitly cast to integer
            if ($oldId) {
                $note = Honoraire::find($oldId); // Use find to handle null gracefully
                if ($note) {
                    $note->act_name = $request->input('old_act_name')[$index] ?? $note->act_name;
                    $note->code = $request->input('old_code')[$index] ?? $note->code;
                    $note->lettrecle = $request->input('old_lettrecle')[$index] ?? $note->lettrecle;
                    $note->date = $request->input('old_date')[$index] ?? $note->date;
                    $note->dent = $request->input('old_dents')[$index] ?? $note->dent;
                    $note->montant = $request->input('old_montant')[$index] ?? $note->montant;
                    $note->save();
                } else {
                    return redirect()->back()->with('error', 'Note not found for ID: ' . $oldId);
                }
            }
        }
    }
    ...
    foreach ($data['act_name'] as $index => $act_id) {
        $noteHonoraireData[] = [
            'act_id' => $act_id,
            'not_id' => $id, // Use the provided $id instead of $note->id
            'act_name' => $act_id ? Act::find($act_id)->name : null, // Handle nullable act_id
            'code' => $data['code'][$index] ?? null, // Handle nullable code
            'lettrecle' => $data['lettrecle'][$index] ?? null, // Handle nullable lettrecle
            'date' => $request->input('date.' . $index, null), // Can be null
    ...
    return redirect()->back()->with('success', 'Notes mises à jour avec succès !');
}
...
{
    dd($id);
    $note = Honoraire::findOrFail($id);
    $note->delete();

    return redirect()->back()->with('success', 'Notes supprimées avec succès !');
}
