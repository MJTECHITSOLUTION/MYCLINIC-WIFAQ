    ...
    <form method="post" action="<?php echo e(route('prescription.store')); ?>">
        <div class="row justify-content-center">
            ...
            <div class="col-md-8">
                                        ...
                                        <tr>
                                            <td>
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="checkbox" name="haut" id="haut" value="Haut">
                                                    <label class="form-check-label" for="haut">Haut</label>
                                                </div>
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="checkbox" name="droite" id="droite" value="Droite">
                                                    <label class="form-check-label" for="droite">Droite</label>
                                                </div>
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="checkbox" name="gauche" id="gauche" value="Gauche">
                                                    <label class="form-check-label" for="gauche">Gauche</label>
                                                </div>
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="checkbox" name="bas" id="bas" value="Bas">
                ...
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">Medicament</h6>
                    </div>
                    <div class="card-body">
...
<?php $__env->startSection('footer'); ?>
    ...
    <script>
        $("#addActe").click(function () {
            ...
            if (selectedActe.val() !== "") {
                ...
                var newRow = "<tr><td>" + ref + "</td><td class='editable-libelle'>" + libelle + "</td><td> <select name='status[]' class='form-control'><option value='En cours'>En cours</option><option value='Terminé'>Terminé</option></select> </td><td>" + dent + "</td><td class='editable-prix'>" + prix + "</td><td>" + numseances + "/" + nums + "</td><td><a class='addObservation'><i class='fas fa-book'></i></a></td><td><button class='deleteActe'><i class='fas fa-trash'></i></button></td><input type='hidden' name='act_id[]' value='" + id + "'><input type='hidden' name='dent[]' value='" + dent + "'><input type='hidden' name='prix[]' value='" + prix + "'><input type='hidden' name='libelle_act[]' value='" + libelle + "'><input type='hidden' name='numseancess[]' value='" + numseances + "'><input type='hidden' name='observationss[]' value=''></tr>";
        ...
        $("#acteTable").on("dblclick", "td.editable-prix", function () {
            var cell = $(this);
    ...
    <script>
        $(document).ready(function() {
            $('#type_med').on('change', function() {
                var typeId = $(this).val();
                console.log(typeId);
                if (typeId) {
                    $.ajax({
                        url: '<?php echo e(route('get.medicaments.by.type')); ?>',
                        type: 'GET',
                        data: { type_id: typeId },
                        success: function(data) {
                            console.log(data);
                            var medicamentSelect = $('#medicament');
                            medicamentSelect.empty();
                            medicamentSelect.append('<option value="">Sélectionner medicament</option>');
                            $.each(data, function(key, value) {
                                medicamentSelect.append('<option value="'+ value.id +'">'+ value.medicament +'</option>');
                            });
                        },
                    ...
                    $('#medicament').empty();
                    $('#medicament').append('<option value="">Sélectionner medicament</option>');
                }
            });
        });
    </script>
...
<?php $__env->stopSection(); ?>
<?php /**PATH /home/u459899255/domains/rmili.myclinic.ma/public_html/resources/views/prescription/specialty/dentiste/create.blade.php ENDPATH**/ ?>