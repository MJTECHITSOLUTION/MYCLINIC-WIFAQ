                                ...
                                <?php $__currentLoopData = $actes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $act): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    ...
                                    <tr class="table-row">
                                        ...
                                        <td><?php echo e($act->prix); ?>
                                            ...
                                            <input type="hidden" name="prix[]" class="prix" id="prix" value="<?php echo e($act->prix); ?>">
                                        </td>
                                        <td>
                                            <input type="text" class="form-control payer" data-row="<?php echo e($loop->index); ?>" name="payer_input[]" id="payer" value="<?php echo e($sumPayer); ?>" readonly>
                                        </td>
                                        <td>
...
<?php $__env->startSection('footer'); ?>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            $('.new_payer').on('input', function() {
                var $row = $(this).closest('.row');
                var montant = parseFloat($row.find('.prix_total').val());
                var payer = parseFloat($row.find('.paye').val());
                var newPayer = parseFloat($(this).val());
                var total_rest = montant - payer;

                // Validate that the total of "payer" and "newPayer" is not greater than "montant"
                if ((payer + newPayer) > montant) {
                    alert("Le montant 'A payer' ne peut pas être supérieur au montant 'Amount'. Il sera réinitialisé.");
                    $(this).val(total_rest);
                }

                // Validate that "newPayer" is not less than zero
                if (newPayer < 0) {
        ...
        $(document).ready(function(){
            $('.payer-input').on('input', function() {
                ...
                var payer = parseFloat(row.find('.payer').val());
                var totalRest = prix - payer;

                if (a_payer + payer > prix) {
...
<?php $__env->stopSection(); ?>
<?php /**PATH /home/u459899255/domains/rmili.myclinic.ma/public_html/resources/views/billing/specialty/dentiste/reglement.blade.php ENDPATH**/ ?>