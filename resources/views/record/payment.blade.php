        ...
        <div class="card-header py-3">
            <div class="row mb-3">
               ...
               <div class="col-6 text-right">
                   ...
                   <br>
                   Rest à payer : {{ $total_payments_range - $restapaye }} {{ App\Setting::get_option('currency') }}
               </div>
            </div>
            <div class="row">
                ...
                <table  class="table table-striped table-bordered table-striped table-responsive{-sm|-md|-lg|-xl|-xxl}"
                    ...
                    @forelse($payments as $payment)
                        <tr>
                            ...
                            <td class="text-center sm__screen"> {{ App\Billing_item::where('billing_id', $payment->id)->sum('payer') }} {{ App\Setting::get_option('currency') }}</td>
                            <td class="text-center sm__screen"> {{ $payment->total_without_tax - App\Billing_item::where('billing_id', $payment->id)->sum('payer') }} {{ App\Setting::get_option('currency') }}</td>
                            <td class="text-center sm__screen"> {{ $payment->total_without_tax }} {{ App\Setting::get_option('currency') }}</td>
                    @empty
                        <tr>
        ...
        <table  class="table table-striped table-bordered table-striped table-responsive{-sm|-md|-lg|-xl|-xxl}"
            ...
            @if(count($payments) > 0)
                <tr class="font-weight-bold">
                    ...
                    <td class="text-center sm__screen">
                        {{ $payments->sum(function($payment) { return App\Billing_item::where('billing_id', $payment->id)->sum('payer'); }) }} 
                        {{ App\Setting::get_option('currency') }}
                    </td>
                    <td class="text-center sm__screen">
                        {{ $payments->sum(function($payment) { return $payment->total_without_tax - App\Billing_item::where('billing_id', $payment->id)->sum('payer'); }) }} 
                        {{ App\Setting::get_option('currency') }}
                    </td>
                    <td class="text-center sm__screen">
                        {{ $payments->sum('total_without_tax') }} 
                        {{ App\Setting::get_option('currency') }}
                    </td>
                </tr>
            @endif
